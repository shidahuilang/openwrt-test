...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  # 本地化改造:直接使用本仓库内 common/ 目录,不再从外部仓库克隆
  sudo rm -rf build/common && cp -Rf common build/common
...
# 本地化改造:common.sh 使用本仓库内文件,不再从 raw.githubusercontent.com 下载
cp -f common/common.sh common.sh
...
# 测试仓库改造:禁用上游同步(tongbu_1/2/3 会用主仓库 openwrt 的 build 配置覆盖本地,
# 导致本仓库的测试改动被清掉),始终使用本仓库自带的 build 配置直接编译
if [[ "${SYNCHRONISE}" == "0" ]]; then
  Diy_continue
else
  echo -e "\033[33m 测试仓库已禁用上游同步,使用本仓库配置继续编译 \033[0m"
  Diy_continue
fi
}
